...
export class MainMenuScene extends Phaser.Scene {
  ...
  private bestLocal(ri: number): number {
    return this.registry.get(`bestLocal_${ri}`) ?? 0;
  }
  private isUnlocked(ri: number): boolean {
    return ri <= 1 || this.bestLocal(ri - 1) >= 20;
  }
  ...
  private drawBackground() {
    ...
    bg.fillCircle(400, 320, 200);
    bg.fillStyle(0xffffff, 0.015);
    bg.fillCircle(400, 320, 280);
    for (let i = 0; i < 30; i++) {
      const sx = Phaser.Math.Between(0, 800),
        sy = Phaser.Math.Between(0, 600);
  ...
  private drawClassCard(
    ...
    els.push(
      this.add
        .text(cx, skillY, cls.skillPreview.join(' · '), {
          fontSize: '11px',
          color: '#88bbdd',
          fontFamily: 'Arial',
          stroke: '#000000',
    ...
    fullZone.on('pointerover', () => {
      drawCard(true);
      drawBtn(true);
    });
    fullZone.on('pointerout', () => {
      drawCard(false);
      drawBtn(false);
    });
  ...
  private showRegionSelection() {
    ...
    els.push(
      this.add
        .text(400, 100, '지역을 선택하여 모험을 시작하세요', {
          fontSize: '14px',
          color: '#8888aa',
          fontFamily: 'Arial, sans-serif',
          stroke: '#000000',
  ...
  private drawRegionCards(els: Phaser.GameObjects.GameObject[]) {
    ...
    REGIONS.forEach((r, i) => {
      ...
      if (unlocked) {
        ...
        topG.fillRoundedRect(cx - cardW / 2, cy - cardH / 2, cardW, 70, {
          tl: 16,
          tr: 16,
          bl: 0,
          br: 0,
        ...
        els.push(topG);
        els.push(
          this.add.text(cx, cy - cardH / 2 + 35, r.icon, { fontSize: '36px' }).setOrigin(0.5),
        );
      } else {
        ...
        els.push(
          this.add
            .text(cx, cy - cardH / 2 + 35, '🔒', { fontSize: '36px' })
            .setOrigin(0.5)
            .setAlpha(0.5),
        );
      }
      ...
      els.push(
        this.add
          .text(cx, cy - 30, `Stage ${stageStart}~${stageEnd}`, {
            fontSize: '14px',
            color: unlocked ? '#aabbcc' : '#444455',
            fontFamily: 'Arial',
            stroke: '#000000',
            strokeThickness: 2,
          })
      ...
      if (cleared) {
        ...
        els.push(
          this.add
            .text(cx, cy + 5, `${prevName} 클리어 시 해금`, {
              fontSize: '12px',
              color: '#555566',
              fontFamily: 'Arial',
              stroke: '#000000',
      ...
      if (unlocked) {
        ...
        zone.on('pointerdown', () => {
          this.cameras.main.fade(
            ...
            (_cam: Phaser.Cameras.Scene2D.Camera, progress: number) => {
              if (progress >= 1)
                this.scene.start('GameScene', { startRegion: ri, classId: this.selectedClassId });
            },
          );
        });
        els.push(zone);
  ...
  private drawAchievementButton(els: Phaser.GameObjects.GameObject[]) {
    ...
    if (unclaimed > 0) {
      ...
      els.push(
        this.add
          .text(bx + bw / 2 - 6, by - bh / 2 + 6, `${unclaimed}`, {
            fontSize: '10px',
            color: '#ffffff',
            fontFamily: 'Arial',
            fontStyle: 'bold',
  ...
  private showAchievementPanel() {
    ...
    for (const cat of categories) {
      ...
      items.forEach((ach, i) => {
        ...
        panelEls.push(
          this.add
            .text(cx + 32, cy + 22, ach.desc, {
              fontSize: '9px',
              color: unlocked ? '#88aacc' : '#444455',
              fontFamily: 'Arial',
              stroke: '#000000',
              strokeThickness: 1,
        ...
        if (unlocked && !claimed) {
          ...
          const rText = this.add
            ...
            .setDepth(504);
          panelEls.push(rText);
          const rZone = this.add
            .zone(rbx + 8, rby, 48, 16)
            .setInteractive({ useHandCursor: true })
            .setDepth(505);
          ...
          panelEls.push(
            this.add
              .text(cx + cellW - 22, cy + cellH - 14, `+${ach.reward}💎`, {
                fontSize: '8px',
                color: '#444455',
                fontFamily: 'Arial',
                stroke: '#000000',
    ...
    const closeZone = this.add
      .zone(closeBx, closeBy, closeBw, closeBh)
      .setInteractive({ useHandCursor: true })
      .setDepth(504);
