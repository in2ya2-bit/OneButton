...
export class TitleScene extends Phaser.Scene {
  ...
  private drawBackground() {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const dx = Phaser.Math.Between(0, 800), dy = Phaser.Math.Between(200, 500);
      dust.fillStyle(0x8888cc, 0.15);
      dust.fillCircle(0, 0, Phaser.Math.Between(2, 5));
      dust.setPosition(dx, dy);
      this.tweens.add({
        targets: dust, x: dx + Phaser.Math.Between(-100, 100), y: dy - Phaser.Math.Between(20, 80),
    ...
    this.tweens.add({
      targets: silhouette, y: -3, duration: 2000,
      yoyo: true, repeat: -1, ease: 'Sine.easeInOut',
    });
  }

  private showIntro() {
    const pressText = this.add.text(400, 480, 'Press Any Key to Start', {
      fontSize: '18px', color: '#888899', fontFamily: 'Arial, sans-serif',
      stroke: '#000000', strokeThickness: 3,
    }).setOrigin(0.5).setAlpha(0);

    const title = this.add.text(400, 160, '⚔ MONSTER\nCLICKER ⚔', {
      fontSize: '52px', color: '#ffd700', fontFamily: 'Arial, sans-serif',
      fontStyle: 'bold', stroke: '#000000', strokeThickness: 8, align: 'center',
      lineSpacing: 8,
    }).setOrigin(0.5).setAlpha(0);

    const subtitle = this.add.text(400, 255, 'Into the Dungeon', {
      fontSize: '16px', color: '#6666aa', fontFamily: 'Arial, sans-serif',
    ...
    this.tweens.add({
      targets: pressText, alpha: 1, duration: 800, delay: 1200,
      onComplete: () => {
        this.tweens.add({
          targets: pressText, alpha: 0.3, duration: 800,
          yoyo: true, repeat: -1, ease: 'Sine.easeInOut',
        });
      },
    });
    ...
    const onAnyKey = () => {
      ...
      this.tweens.add({
        targets: [pressText], alpha: 0, duration: 200,
        onComplete: () => { pressText.destroy(); this.showMainMenu(); },
      });
    };
  ...
  private showMainMenu() {
    ...
    interface MenuEntry { label: string; icon: string; enabled: boolean; badge?: number; action: () => void }
    const items: MenuEntry[] = [
      ...
      { label: '업적', icon: '🏆', enabled: true, badge: unclaimed > 0 ? unclaimed : undefined, action: () => this.goToAchievements() },
      { label: '설정', icon: '⚙️', enabled: true, action: () => this.showSettings() },
    ];

    const startY = 320, gap = 48, btnW = 220, btnH = 40;
    ...
    items.forEach((item, i) => {
      ...
      const t = this.add.text(400, by, `${item.icon}  ${item.label}`, {
        fontSize: '17px', color: labelColor, fontFamily: 'Arial, sans-serif',
      ...
      if (item.badge) {
        ...
        els.push(bdg);
        const bdgT = this.add.text(400 + btnW / 2 - 12, by - 10, `${item.badge}`, {
      ...
      if (!item.enabled && item.label === '이어하기') {
        const sub = this.add.text(400, by + 14, '저장 데이터 없음', {
          fontSize: '9px', color: '#333344', fontFamily: 'Arial',
      ...
      if (item.enabled && hasRun && item.label === '이어하기') {
        ...
        const sub = this.add.text(400, by + 14, `${cls?.icon ?? ''} Stage ${rd.stage}`, {
          fontSize: '9px', color: '#6688aa', fontFamily: 'Arial',
          stroke: '#000000', strokeThickness: 1,
        }).setOrigin(0.5).setAlpha(0);
        els.push(sub);
      ...
      this.tweens.add({
        targets: [g, t], alpha: 1, x: '+=0',
        duration: 300, delay: 300 + i * 80, ease: 'Sine.easeOut',
      });
      g.setPosition(-30, 0);
      t.setPosition(t.x - 30, t.y);
      this.tweens.add({
        targets: [g], x: 0, duration: 350, delay: 300 + i * 80, ease: 'Back.easeOut',
      });
      this.tweens.add({
        targets: [t], x: '+=30', duration: 350, delay: 300 + i * 80, ease: 'Back.easeOut',
      });
      ...
      if (item.enabled) {
        ...
        zone.on('pointerdown', () => { SoundManager.sfxClick(); item.action(); });
        els.push(zone);
      }
    });

    const ver = this.add.text(790, 590, 'v1.0', {
      fontSize: '10px', color: '#333344', fontFamily: 'Arial',
    }).setOrigin(1, 1);
    els.push(ver);
  }
  ...
  private continueRun() {
    ...
    SoundManager.stopBgm();
    this.cameras.main.fade(300, 0, 0, 0, false, (_c: Phaser.Cameras.Scene2D.Camera, p: number) => {
      if (p >= 1) this.scene.start('GameScene', { startRegion: rd.startRegion, classId: rd.classId, loadRun: true });
    });
  }
  ...
  private showSettings() {
    ...
    sEls.push(this.add.text(400, 150, '⚙️ 설정', {
    ...
    const makeSlider = (label: string, y: number, val: number, onChange: (v: number) => void) => {
      sEls.push(this.add.text(240, y, label, {
        fontSize: '14px', color: '#aabbcc', fontFamily: 'Arial',
        stroke: '#000000', strokeThickness: 2,
      }).setDepth(502));

      const sx = 350, sw = 180, sh = 8;
      const track = this.add.graphics().setDepth(502);
      track.fillStyle(0x222233, 1); track.fillRoundedRect(sx, y + 6, sw, sh, 4);
      sEls.push(track);

      const fill = this.add.graphics().setDepth(503);
      const knob = this.add.graphics().setDepth(504);
      const valText = this.add.text(sx + sw + 15, y + 2, `${Math.round(val * 100)}%`, {
      ...
      const drawSlider = (v: number) => {
        fill.clear(); knob.clear();
        const fw = sw * v;
        fill.fillStyle(0x4466aa, 1); fill.fillRoundedRect(sx, y + 6, fw, sh, 4);
        knob.fillStyle(0xffffff, 1); knob.fillCircle(sx + fw, y + 10, 8);
        valText.setText(`${Math.round(v * 100)}%`);
      };
      drawSlider(val);

      const zone = this.add.zone(sx + sw / 2, y + 10, sw + 20, 30).setInteractive().setDepth(505);
      zone.on('pointerdown', (ptr: Phaser.Input.Pointer) => {
        const v = Phaser.Math.Clamp((ptr.x - sx) / sw, 0, 1);
        drawSlider(v); onChange(v);
      });
    ...
    const muteBx = 400, muteBy = 315;
    const muteG = this.add.graphics().setDepth(502);
    const muteText = this.add.text(muteBx, muteBy, '', {
    ...
    const muteZone = this.add.zone(muteBx, muteBy, 140, 32).setInteractive({ useHandCursor: true }).setDepth(504);
    ...
    const fsBx = 400, fsBy = 365;
    const fsG = this.add.graphics().setDepth(502);
    const fsText = this.add.text(fsBx, fsBy, '', {
      fontSize: '14px', color: '#ffffff', fontFamily: 'Arial', fontStyle: 'bold',
    }).setOrigin(0.5).setDepth(503);
    ...
    const fsZone = this.add.zone(fsBx, fsBy, 140, 32).setInteractive({ useHandCursor: true }).setDepth(504);
    ...
    drawClose(false);
    sEls.push(closeG);
    sEls.push(this.add.text(400, closeBy, '닫기', {
      fontSize: '15px', color: '#ffffff', fontFamily: 'Arial', fontStyle: 'bold',
    }).setOrigin(0.5).setDepth(503));
    const closeZone = this.add.zone(400, closeBy, 100, 32).setInteractive({ useHandCursor: true }).setDepth(504);
