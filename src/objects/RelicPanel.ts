...
export class RelicPanel {
  ...
  private get pts(): number {
    return this.scene.registry.get('relicPoints') ?? 0;
  }
  private set pts(v: number) {
    this.scene.registry.set('relicPoints', v);
  }
  ...
  private draw() {
    ...
    panel.fillStyle(0x0c0c1e, 0.97);
    panel.fillRoundedRect(28, 14, 744, 572, 12);
    panel.lineStyle(2, 0x4433aa, 0.7);
    panel.strokeRoundedRect(28, 14, 744, 572, 12);
    e.push(panel);
  ...
  private drawTabs(D: number) {
    const s = this.scene,
      e = this.els;
    const tabs: [string, 'upgrade' | 'refund'][] = [
      ['강화', 'upgrade'],
      ['포인트 환불', 'refund'],
    ];
  ...
  private drawGrid(D: number) {
    ...
    RELICS.forEach((relic, i) => {
      ...
      if (s.textures.exists(texKey)) {
        ...
        e.push(
          s.add
            .text(iconX, cy - 8, relic.icon, { fontSize: '20px' })
            .setOrigin(0.5)
            .setDepth(D + 2),
        );
      }
      ...
      e.push(
        s.add
          .text(cx + 8, cy - 14, relic.name, {
            fontSize: '11px',
            color: lv > 0 ? '#ffffff' : '#777788',
            fontFamily: 'Arial',
            fontStyle: 'bold',
            stroke: '#000000',
            strokeThickness: 2,
      ...
      for (let b = 0; b < MAX_RELIC_LV; b++) {
        ...
        bg2.lineStyle(1, 0x333344, 0.5);
        bg2.strokeRect(bx, barY, 8, 5);
        e.push(bg2);
      }
  ...
  private drawDetail(D: number) {
    ...
    if (s.textures.exists(texKey)) {
      ...
      e.push(
        s.add
          .text(88, py + 30, relic.icon, { fontSize: '28px' })
          .setOrigin(0.5)
          .setDepth(D + 2),
      );
    }
    ...
    e.push(
      s.add
        .text(130, py + 38, curStr, {
          fontSize: '12px',
          color: lv > 0 ? '#88ddaa' : '#555566',
          fontFamily: 'Arial',
          stroke: '#000000',
          strokeThickness: 2,
        })
    ...
    const stars =
      lv > 0 ? '★'.repeat(lv) + '☆'.repeat(MAX_RELIC_LV - lv) : '☆'.repeat(MAX_RELIC_LV);
    e.push(
      s.add
        .text(130, py + 78, stars, {
          fontSize: '12px',
          color: isMax ? '#ddaa33' : lv > 0 ? '#ffcc00' : '#444444',
          fontFamily: 'Arial',
        })
        .setDepth(D + 2),
    );
    ...
    if (this.tab === 'upgrade') {
      if (isMax) {
        ...
        const drawBtn = (hover: boolean) => {
          ...
          btnG.fillStyle(fc, 1);
          btnG.fillRoundedRect(600, py + 46, 80, 32, 8);
          btnG.lineStyle(2, bc, 1);
